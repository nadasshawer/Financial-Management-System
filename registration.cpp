...
bool checkLetterMonthInput(string letterMonth) {
	unordered_set<string>monthsOfYear = {"january",
	                                     "february",
	                                     "march",
	                                     "april",
	                                     "may",
	...
	transform(letterMonth.begin(), letterMonth.end(), letterMonth.begin(), [](char c) {
		return tolower(c);
	});
...
bool checkPassword() {
	...
	unordered_set<char> specialChar = {'!', '@', '#', '$', '%', '^', '&', '*', '(', ')', '_', '+', '=', '-', '{', '}', ':', ';', '\'', '/', '|', '~', '<', '>', '.', ',', '?'};
	...
	if (firstPassword.size() >= 8) {
		if(hasCapLetter && hasSmallLetter && hasSpecialChar && hasNum) {
			...
			cout << "Password has to include at least one capital letter. Retry -->  ";
			return false;
		} else {
